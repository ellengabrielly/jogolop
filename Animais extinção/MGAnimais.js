...
function preload() {
    ...
    asas = loadImage('asas.png');
    praia = loadImage('praia.png');
    oceano = loadImage('oceano.png');
    chaveTESTE = loadImage('chaveTESTE.png');
...
function draw() {
    ...
    */
    ...
    if (tela == 4) { //arara
        ...
        for (i = 0; i < tirosAV; i++) {
            if (tirosBS[i]) {
                image(chaveTESTE, tirox[i], tiroy[i] + 35, 15, 15);
                tirox[i] += 20;
                if (tirox[i] > width) {
                    tirox[i] = 10000000;
                    tirosBS[i] = false;
    ...
    if (tela == 30) {
        ...
        textSize(30); //muda só o tamanho do texto
        text("Aperte ENTER para voltar a tela inicial", 250, 250);
        vida = 3;
        pontuacao = 0;
        ans = 0;
        if (keyIsDown(ENTER)) {
    ...
    if (tela == 40) {
        ...
        textFont(font, 60); //determina uma fonte 
        fill(206, 63, 59); //escolhe a cor do texto
        text("Você venceu!!!", 150, 220); // alert o texto com suas coordenadas
        textSize(15); //muda só o tamanho do texto
        text("Aperte CONTROL para jogar novamente", 350, 400);
        vidas = 3;
        pontuacao = 0;
        ans = 0;
